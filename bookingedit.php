...
$errorMessage = "";
$student_id = $tutor_id = $booking_date = $payment_status = $booking_id = "";

if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // Check if booking_id is set in GET parameters
    ...
    if (empty($booking_id) || empty($student_id) || empty($tutor_id) || empty($booking_date) || empty($payment_status)) {
        $errorMessage = "All fields are required!";
    } else {
        // Use prepared statements to prevent SQL injection
        $sql = "UPDATE booking SET student_id = ?, tutor_id = ?, booking_date = ?,payment_status = ?  WHERE booking_id = ?";
        $stmt = $connection->prepare($sql);
        $stmt->bind_param("sssii", $student_id, $tutor_id, $booking_date, $payment_status, $booking_id);
    ...
    <center>
        ...
        <form method="POST" onsubmit="return confirmUpdate();">
            ...
            <div>
                ...
                <input class="em" type="text" name="booking_date" value="<?php echo $booking_date ; ?>" placeholder="Enter Booking Date">
            </div>
            <div>
                <label for="payment_status">payment_status</label>
                <input class="em" type="text" name="payment_status" value="<?php echo $payment_status ; ?>" placeholder="Enter payment_status">
            </div>
            <div>
